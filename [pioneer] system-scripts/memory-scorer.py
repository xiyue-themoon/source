...
def cmd_score(filepath: str):
    ...
    suggested = math.ceil(total * 1.3 / LIMIT_STEP) * LIMIT_STEP
    print(f"📈 总大小: {total} chars")
    print(f"   扩容建议: {suggested} (总内容×1.3={total*1.3:.0f}, step={LIMIT_STEP})")
    return 0
...
def cmd_expand(filepath: str):
    ...
    suggested = math.ceil(current_total * 1.3 / LIMIT_STEP) * LIMIT_STEP
    ...
    print(f"  (总内容×1.3={current_total*1.3:.0f}, 取整到 {LIMIT_STEP})")
    print()
    print(f"当前各 tier 占比:")
    for tier in ["P0", "P1", "P2"]:
        t = stats[tier]
